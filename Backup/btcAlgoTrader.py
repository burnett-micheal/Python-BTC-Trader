...
# TODO: Restructure Data In QuoteHandler, {'minute samples':{sample size:{'moving avg':{'price':[], 'date':[]}, 'standard deviation':{'price':[], 'date':[]}  }}}
# TODO: Update Algo To Meet Minimum APY
# TODO: Write Each Trade In BtHistory To File At The End Of BackTest Function
# TODO: Review Backtest For Validity
...
def backtest(dateStart, dateEnd):
  ...
  totalCash = tradeHandler._btCash
  prevValue = totalCash
  while(curDate < dateEnd):
    ...
    if(os.path.isfile(filePath)):
      ...
      holdingsValue = 0
      for hKey in tradeHandler.holdings: holdingsValue += tradeHandler.holdings[hKey]['qty'] * tradeHandler.holdings[hKey]['ap']
      newValue = holdingsValue + tradeHandler._btCash
      cm = CashMath()
      interest = math.floor(cm.interest(prevValue, newValue) * 10000) / 100
      print(interest)
      print(" - - - - - - - - - - - - - - - - - - ")
      prevValue = newValue
...
# Up Trend Backtest
backtest(dc.date(2022, 2, 2, 0, 0, 0, 0), dc.date(2022, 2, 4, 0, 0, 0, 0))
